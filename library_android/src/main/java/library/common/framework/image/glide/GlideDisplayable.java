...
public class GlideDisplayable implements Displayable {
    ...
    @Override
    public void displayRoundedCorners(Context context, ImageView imageView, Drawable drawable, int radius, int placeHolderId, int errorHolderId, RoundedCornersTransformation.CornerType cornerType) {
        Glide.with(context).load(drawable).listener(mListener).apply(mRequestOptions != null ? mRequestOptions : GlideOptions.withRoundedOptions(radius, placeHolderId, errorHolderId, cornerType)).into(getTarget(imageView));
    }
    ...
    @Override
    public void displayRoundedCorners(Context context, ImageView imageView, String url, int radius, int placeHolderId, int errorHolderId, RoundedCornersTransformation.CornerType cornerType) {
        Glide.with(context).load(decorateUrl(url)).listener(mListener).apply(mRequestOptions != null ? mRequestOptions : GlideOptions.withRoundedOptions(radius, placeHolderId, errorHolderId, cornerType)).into(getTarget(imageView));
    }
    ...
    @Override
    public void displayRoundedCorners(Context context, ImageView imageView, String url, int radius, RoundedCornersTransformation.CornerType cornerType) {
        displayRoundedCorners(context, imageView, url, radius, -1, -1, cornerType);
    }
